...
Rpm =PMFG_T2s(S);
t = toc;
fprintf('TMFG time: %.5f  \n',t)

[r,c,val] = find(Rpm);
fprintf('edge_sum: %f \n', sum(val));

Apm=Rpm;Apm(Apm~=0)=D(Apm~=0); %APM: THE tmfg Matrix with weights
tic;
...
CliqList=clique;
clear clique
for n=1:Nc; % see if a clique is a seprating clique
    cliq_vec=CliqList(n,:);
...
d(IndxNot(1))=0;
Indx1= d==-1;Indx2= d~=-1;
T(Indx1)=1;T(Indx2)=2;T(Cliq)=0; % 1 and 2: if reachable == in or out of the clique
clear Temp
end
